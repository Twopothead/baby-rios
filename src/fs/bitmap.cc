...
void format_inode_bitmap_blks(union Super_Block_Sect  rios_superblock){
	u8 sector[512]={0};
	memset(sector,0x00,sizeof(sector)/sizeof(sector[0]));/*aa*/
...
void format_inode_blks(union Super_Block_Sect  rios_superblock){
	u8 sector[512]={0};
	memset(sector,0x00,sizeof(sector)/sizeof(sector[0]));/*bb*/
...
{
	u8 sector[512]={0};
	memset(sector,0x00,sizeof(sector)/sizeof(sector[0]));/*cc*/
	// for(int i=NR_INODE_MAP_BLK(rios_superblock);i<;i++)
	// IDE_write_sector((void *)&sector,i);
}
void new_block(){
